...
function EFFECT:DoFX( pos, vel )
	...
	if emitter then
		...
		if particle then
			...
			particle:SetEndSize( math.Rand(60,80)*(3+(math.min(vel:Length(),600) / 600) * 2) )
			particle:SetRoll( table.Random({-1,1}) * 100 )
		...
		if particle then
			...
			particle:SetDieTime( 1 - (math.min(vel:Length(),600) / 600) * 0.5 )
			particle:SetAirResistance( 0 ) 
			particle:SetStartAlpha( 255 )
			particle:SetStartSize( math.Rand(10,14)*4 )
			particle:SetEndSize( math.Rand(5,7)*4 )
			particle:SetRoll( table.Random({-1,1}) * 180 )
		...
		for i = 0,3 do
			...
			if particle then
				particle:SetVelocity( Vector(0,0,70) + vel )
				particle:SetDieTime( 0.75 - (math.min(vel:Length(),600) / 600) * 0.375 )
				particle:SetAirResistance( 0 ) 
				particle:SetStartAlpha( 255 )
				particle:SetStartSize( math.Rand(10,14)*3 )
				particle:SetEndSize( math.Rand(5,7)*3 )
				particle:SetRoll( table.Random({-1,1}) * 180 )
				particle:SetColor( 255,255,255 )
				particle:SetGravity( Vector( 0, 0, 70 ) )
				particle:SetCollide( false )
